...
def make_history(
    *,
    ...
    return PriceHistory(
        ...
        rows=rows
        or (
            PriceRecord(date="2024-01-01", close=100.0),
            PriceRecord(date="2025-01-01", close=110.0),
            PriceRecord(date="2026-01-01", close=121.0),
        ),
    )
...
async def test_analyze_ticker_calls_downloader_and_returns_analysis():
    ...
    result = await service.analyze_ticker(
        "aapl",
        period="2y",
        interval="1mo",
    )

    client.get_price_history.assert_awaited_once_with(
        "aapl",
        period="2y",
        interval="1mo",
    )
...
def test_analyze_history_calculates_available_moving_averages():
    rows = tuple(
        PriceRecord(
            date=f"2024-{((index - 1) % 12) + 1:02d}-{((index - 1) // 12) + 1:02d}",
            close=float(index),
        )
        for index in range(1, 201)
    )
    # Use valid, strictly increasing dates for the service date checks.
    rows = tuple(
        PriceRecord(
            date=f"{2000 + ((index - 1) // 12):04d}-{((index - 1) % 12) + 1:02d}-01",
            close=float(index),
        )
        for index in range(1, 201)
    )
    ...
    result = service._analyze_history(
        make_history(interval="1mo", rows=rows)
    )
...
def test_calculate_years_rejects_non_increasing_dates():
    ...
    with pytest.raises(InvalidPriceHistoryError):
        AnalyzerService._calculate_years(
            start_date=parsed,
            end_date=parsed,
        )

...
@pytest.mark.asyncio
async def test_analysis_endpoint_returns_json_and_forwards_query(aiohttp_client):
    service = AsyncMock()
    ...
    response = await client.get(
        "/analysis/aapl?period=5y&interval=1wk"
    )
    body = await response.json()

    assert response.status == 200
    service.analyze_ticker.assert_awaited_once_with(
        "aapl",
        period="5y",
        interval="1wk",
    )
...
@pytest.mark.parametrize(
    ...
    [
        (
            DownloaderResponseError(404, "Unknown ticker."),
            404,
            "downloader_request_error",
        ),
        (
            DownloaderResponseError(500, "Downloader failed."),
            502,
            "downloader_error",
        ),
        (
            InvalidDownloaderResponseError("Bad payload."),
            502,
            "invalid_downloader_response",
        ),
        (
            DownloaderClientError("Unavailable."),
            503,
            "downloader_unavailable",
        ),
        (
            InvalidPriceHistoryError("Not enough rows."),
            400,
            "calculation_error",
        ),
    ],
)
async def test_analysis_endpoint_translates_errors(
    aiohttp_client,
    error,
    status,
    error_code,
